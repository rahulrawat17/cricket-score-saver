...
const ScoreBoard = (toggleComponent) => {
  ...
  const [isRunOut, setRunOut] = useState(false)
  ...
  var maxOver = parseInt(data.maxOver)
  ...
  const handleChangeMatchOver = (e) => {
    document.getElementById('change-match-over-submit').style.display = "unset";
    document.getElementById('change-match-over-input').style.display = "unset";
  }

  const handleChangeMatchOverInputSubmit = (e) => {
    maxOver = document.getElementById('change-match-over-input').value || maxOver
    document.getElementById('change-match-over-submit').style.display = "none";
    document.getElementById('change-match-over-input').style.display = "none";
    // 
    let data2 = JSON.parse(localStorage.getItem('data'))
    data2.maxOver = maxOver
  ...
  const overCompleted = (runsByOverParam, currentRunStackParam) => {
    ...
    if (overCount + 1 === maxOver) {
      ...
      const changeOverBtn = document.getElementById('change-match-over')
      changeOverBtn.disabled = true
    } else {
      bowlerNameElement.disabled = false
    }
  ...
  const undoRun = (run, isNoBallParam) => {
    if (isNoBallParam) {
      // noballchanges
      setTotalRuns(totalRuns - (run + 0))
      // noballchanges
      setRunsByOver(runsByOver - (run + 0))
    } else {
  ...
  const handleNoBall = () => {
    ...
    // noballchanges
    setRunsByOver(runsByOver + 0)
    // noballchanges
    setExtras((state) => ({
      ...state,
      total: state.total + 0,
      noBall: state.noBall + 0,
    }))
    addNoBallEffect()
  }
  // no ball effect
  ...
  const handleRunOut = () => {
    if (inningNo === 2) {
      setRemainingRuns(remainingRuns)
    }
    // noballchanges
    setTotalRuns(totalRuns)
    // noballchanges
    setRunsByOver(runsByOver)
    // noballchanges
    setExtras((state) => ({
      ...state,
      total: state.total,
  ...
  // run out effect
  const addRunOutEffect = () => {
    const scoreTypesButtons = document.querySelectorAll('.score-types-button')
    for (let i = 0; i < scoreTypesButtons.length; i++) {
      scoreTypesButtons[i].classList.add('score-types-button-runOut')
    }
    setRunOut(true)
  }
  const removeRunOutEffect = () => {
    const scoreTypesButtons = document.querySelectorAll('.score-types-button')
    for (let i = 0; i < scoreTypesButtons.length; i++) {
      scoreTypesButtons[i].classList.remove('score-types-button-runOut')
    }
    setRunOut(false)
  }
  ...
  return (
    <div className='container'>
      <div className='inning'>
        ...
        <div>
          <input id="change-match-over-input" style={{display: 'none'}} type="number" />
          <button id="change-match-over-submit" style={{display: 'none'}} onClick={handleChangeMatchOverInputSubmit}>OK</button>
          <button id='change-match-over' style={{display: inningNo === 2 ? 'none' : 'unset'}} onClick={handleChangeMatchOver}>
            Change Match Over
          </button>
          ...
          <table>
            <tbody>
              <tr>
                ...
                <td className='score-types' onClick={() => setModalOpen(true)}>
                  <button className='score-types-button' disabled>
                    W
                  </button>
                </td>
              ...
              <tr>
                ...
                <td className='score-types' onClick={handleRetired}>
                  <button className='score-types-button' disabled>
                    Rt.
                  </button>
                </td>
